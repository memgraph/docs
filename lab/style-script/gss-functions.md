...
Returns a single value generated by reducing an array of values with a reduce function.
A function has two parameters, `previous reduced value` and `current array value`. The
third parameter is the initial value used for the first reduce iteration.
