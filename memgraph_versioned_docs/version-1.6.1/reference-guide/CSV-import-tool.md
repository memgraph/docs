...
splitting the raw CSV value using the array delimiter character.
...
The `--nodes` and  `--relationships` flags are used to specify CSV files that contain the nodes nodes and relationships 
...
The format of `--relationships` flag is:
`[<type>=]<file>[,<file>][,<file>]...`.  Take note that only the first `<file>` part
