...
- Added `init_file` and `init_data_file` configuration flags. With configuration flag `init_file` you can execute queries from cypherl file which need to be executed before bolt server starts and with configuration flag `init_data_file` you can execute queries from cypherl file immediately after bolt server starts.[655]()
