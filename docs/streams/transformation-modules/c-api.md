...
```
Returns the payload size argument `message`.
...
```
Returns the payload of argument `message` as a byte array with size `mgp_message_payload_size(message)`.
...
```
Returns topic name of argument `message`. Topic name is NUL terminated.
...
Note that Unicode characters are not allowed. Additionally, a `name` is case-sensitive.
