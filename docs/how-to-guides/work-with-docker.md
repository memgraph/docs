...
Memgraph chose Docker because it is:
...
If you are new to Docker this guide will help you get a grasp on Docker and make
it easier to accomplish tasks within Memgraph. After installing Docker, all
...
- `memgraph-platform` - installs the whole Memgraph Platform, which includes
  **MemgraphDB**, command-line interface for running queries **mgconsole**,
  visual user interface for running queries and visualizing graph data
  **Memgraph Lab** and graph algorithms and modules library **MAGE**
- `memgraph-mage` - installs **MemgraphDB** and graph algorithms and modules
  library **MAGE**
...
Images are download using a `docker pull` command followed by the name of the
Docker image. We encourage you to use Memgraph Platform as it includes
everything you might need while making the best of Memgraph.

To download the Memgraph Platform image run:
...
All images are started using the `docker run` command followed by various
flags, environmental variables and configuration options.

The most common flags used while running Memgraph images are used to:
- enable interactive mode: `-it`
- publish ports: `-p 3000:3000`
- specify volumes `-v mg_lib:/var/lib/memgraph`
- set up configuration using environmental variables in the case of
  `memgraph-platform` image, or configuration flags using `memgraph` or
  `memgraph-mage` image

So a `docker run` command can look like this:
...
separated by a colon. The left side port stands for the **host port** and the right
side port is the **container port**. 

The most common ports published while running Memgraph are:
- `-p 7687:7687` - connection to the database instance
- `-p 3000:3000` - connection to the instance via Memgraph Lab
- `-p 7444:7444` - connection to logs from Memgraph Lab 2.0

For example, you are running two instances using `memgraph-platform` image and
you want to connect to both instances using Memgraph Lab in-browser application.

You would run the first instance with:

```
docker run -it -p 7687:7687 -p 3000:3000 memgraph/memgraph-platform
```

Because ports 7687 and 3000 are now taken, you need to change the left side ports:

```
docker run -it -p 7688:7687 -p 3001:3000 memgraph/memgraph-platform
```

To connect to the first instance you should open Memgraph Lab in browser at
`localhost:3000`, but the second instance at `localhost:3001`.
...
The exact location of the local directories depend on your specific setup.

The configuration file can usually be find at
`/var/lib/docker/volumes/mg_etc/_data/memgraph.conf` but you can also copy the
file from the Docker, modify it and copy it back into Docker.

 Logs can usually be find in
 `\\wsl$\docker-desktop-data\version-pack-data\community\docker\volumes\` but
 you can also view them in the Memgraph Lab 2.0 by publishing the port `7444`.
...
`docker stop`. To stop a container you need [to know the container's ID](#how-to-retrieve-docker-container-id).
...
## How to retrieve Docker container ID?
...
## How to retrieve Docker container IP address?

To retrieve the Docker container IP address, first you need [to retrieve it's ID](#how-to-retrieve-docker-container-id).
...
To browse files inside a Docker container, first you need [to retrieve it's ID](#how-to-retrieve-docker-container-id).
...
To navigate through the container:
...
To copy files from and to the Docker container, first you need [to retrieve it's ID](#how-to-retrieve-docker-container-id).
