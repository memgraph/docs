...
For maps there are 5 fields available: `hour`, `minute`, `second`, `millisecond`
and `microsecond`.
...
documentation on the [Pyhon
API](/reference-guide/query-modules/api/python-api.md) and [C
API](/reference-guide/query-modules/api/c-api/overview.md) pages.
