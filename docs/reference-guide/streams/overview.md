...
**Memgraph Community** doesn't support authentication and authorization, so
the owner ia always `Null`, and the privileges are not checked. 
...
2. Every message is processed **at most once**: the message offsets are
   ...
   processing is started. This means if the processing fails, the same
...
A stream can fail for various reasons. One of important failures is when a
...
execute the transaction again for up to `TIMES_TO_RETRY` times (default
value is 30).
